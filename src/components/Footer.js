...
function SocialLink({ className, href, icon }) {
  return (
    <Link
      className={clsx(
        'flex h-10 w-10 items-center justify-center rounded-full bg-blue-500 duration-300 ease-in-out hover:bg-blue-600',
        className,
      )}
      href={href}
    >
...
export const Footer = ({ programs, contact }) => {
  return (
    <footer className="space-y-8 divide-y divide-blue-400/20 bg-blue-100 px-4 pt-16 sm:px-6 sm:pt-20 lg:px-8">
      {/* Top section: blocks */}
      <div className="mx-auto grid max-w-md gap-y-8 sm:max-w-none sm:grid-cols-2 sm:gap-x-8 sm:gap-y-12 md:gap-x-12 lg:max-w-screen-2xl lg:grid-cols-11 lg:gap-8 xl:gap-12">
        ...
        <div className="flex flex-col lg:col-span-4 lg:mx-auto">
          ...
          <div className="flex items-center">
            <div className="w-60 flex-shrink-0 flex-grow-0">
              <Link href="/">
                <Image src={logo} alt="Summit Learning" className="h-auto" />
              </Link>
            </div>
          </div>
          {/* Mission statement */}
        ...
        <div className="flex-shrink sm:order-3 lg:order-none lg:col-span-2">
          <h6 className="relative text-xl font-bold tracking-wide text-blue-900">
            <span className="relative z-20">Programs</span>
            <span className="absolute -bottom-1 left-0 z-10 h-1 w-12 rounded-lg bg-gradient-to-r from-blue-400 to-blue-500" />
          </h6>
          {/* Program links */}
          <ul className="mt-6 divide-y divide-blue-400/20 text-lg">
